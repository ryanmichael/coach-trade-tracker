...
// Alias global fetch to avoid shadowing by the store's `fetch` method
...
export const useDelistMonitorStore = create<DelistMonitorState>((set, get) => ({
  ...
  fetch: async () => {
    ...
    try {
      const res = await apiFetch("/api/delist-monitor");
  ...
  removeTicker: async (id: string) => {
    ...
    try {
      const res = await apiFetch(`/api/delist-monitor/${id}`, { method: "DELETE" });
      if (!res.ok) {
        // Revert on failure
        await get().fetch();
      }
  ...
  runCheck: async () => {
    ...
    try {
      const res = await apiFetch("/api/delist-monitor/check", { method: "POST" });
  ...
  fetchHistory: async (id: string) => {
    try {
      const res = await apiFetch(`/api/delist-monitor/${id}/history`);
      if (!res.ok) return [];
