...
export async function GET() {
  ...
  const userFilter = user ? { userId: user.id } : { userId: null as string | null };
  ...
  const totalPredictions = await prisma.optionsSnapshot.count({ where: userFilter });
  const pendingValidation = await prisma.optionsSnapshot.count({
    where: { ...userFilter, validatedAt: null },
  });
